...
for i in xrange(rows-1):
    for j in xrange(cols-1):
        ...
        new_r[i][j+1]       = np.uint8(new_r[i][j+1] + int(errR*(5/16.0)))
        new_r[i+1][j]       = np.uint8(new_r[i+1][j] + int(errR*(7/16.0)))
        new_r[i+1][j+1]    = np.uint8(new_r[i+1][j+1] + int(errR*(1/16.0)))
        new_r[i-1][j+1]    = np.uint8(new_r[i+1][j+1] + int(errR*(3/16.0)))

        new_g[i][j]           = avg_g[closestBoxIndex]
        new_g[i][j+1]       = np.uint8(new_g[i][j+1] + int(errG*(5/16.0)))
        new_g[i+1][j]       = np.uint8(new_g[i+1][j] + int(errG*(7/16.0)))
        new_g[i+1][j+1]    = np.uint8(new_g[i+1][j+1] + int(errG*(1/16.0)))
        new_g[i-1][j+1]    = np.uint8(new_g[i+1][j+1] + int(errG*(3/16.0)))

        new_b[i][j]           = avg_b[closestBoxIndex]
        new_b[i][j+1]       = np.uint8(new_b[i][j+1] + int(errG*(5/16.0)))
        new_b[i+1][j]       = np.uint8(new_b[i+1][j] + int(errG*(7/16.0)))
        new_b[i+1][j+1]    = np.uint8(new_b[i+1][j+1] + int(errG*(1/16.0)))
        new_b[i-1][j+1]    = np.uint8(new_b[i+1][j+1] + int(errG*(3/16.0)))
