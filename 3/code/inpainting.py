...
def pixel_gradient(y, x, height, width, vals, flags):
    val = vals[y, x]
...
def compute_outside_dists(height, width, dists, flags, band, radius):
    band_c = []
    band_c.extend(band)
    # band = band.copy()
    band = band_c
...
def init(height, width, mask, radius):
    ...
    compute_outside_dists(height, width, dists, flags, band, radius)

    return dists, flags, band

# returns RGB values for pixel to by inpainted, computed for its neighborhood
def inpaint_pixel(y, x, img, height, width, dists, flags, radius):
    dist = dists[y, x]
