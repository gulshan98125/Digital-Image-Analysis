...
def isSameSide(p1,p2,p3,p4):
	y1,y2 = p1[1],p2[1]
	x1,x2 = p1[0],p2[0]
	line = lambda p: (p[0]-x1) - ((x2-x1)/(y2-y1))*(p[1]-y1)
	if(line(p3)*line(p4)>0):
		return True
	else:
		return False
...
def topFlatFill(P1,P2,P3,P1lam,P2lam,P3lam, dictA, imgLeft, imgRight,t): #filling using the middle image triangle
	...
	y = P1[1]
	while(y >= P3[1]):
		leftmostX = np.ceil(P1[0] + (y-P1[1])*((P3[0]-P1[0])/(P3[1]-P1[1])))
		# if(isSameSide(P1,P3,P2,(leftmostX,y))):
		# 	# print "leftmostX insdie"
		# 	leftmostX = int(leftmostX)
		# else:
		# 	# print "leftmostX outside"
		# 	leftmostX = int(leftmostX)+1
		rightmostX = np.floor(P2[0] + (y-P2[1])*((P3[0]-P2[0])/(P3[1]-P2[1])))
		# if(isSameSide(P2,P3,P1,(rightmostX,y))):
		# 	# print "rightmostX insdie"
		# 	rightmostX = int(rightmostX)
		# else:
		# 	# print "rightmostX outside"
		# 	rightmostX = int(rightmostX)-1
		x = leftmostX
		while(x <= rightmostX):
			...
			if(newB<0):
				newB=0
			if(int(x)==501 and int(y)==275):
				print [np.uint8(newR),np.uint8(newG),np.uint8(newB)]
...
def bottomFlatFill(P1,P2,P3,P1lam,P2lam,P3lam,dictA,imgLeft,imgRight,t):
	...
	y = P2[1]
	while(y<=P1[1]):
		leftmostX = np.ceil(P1[0] + (y-P1[1])*((P2[0]-P1[0])/(P2[1]-P1[1])))
		# if(isSameSide(P1,P2,P3,(leftmostX,y))):
		# 	leftmostX = int(leftmostX)
		# else:
		# 	leftmostX = int(leftmostX)+1
		# #print("leftmostX",leftmostX)
		rightmostX = np.floor(P1[0] + (y-P1[1])*((P3[0]-P1[0])/(P3[1]-P1[1])))
		# if(isSameSide(P1,P3,P2,(rightmostX,y))):
		# 	rightmostX = int(rightmostX)
		# else:
		# 	rightmostX = int(rightmostX)-1
