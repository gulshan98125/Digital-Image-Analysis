...
DIR = "../data/"
filenames = ['celeb.jpg','gigi.jpg']
background = cv2.imread(DIR + filenames[0],1)
height,width,_ = background.shape
temp = [[[0,0,0]]*width]*height
temp = np.array(temp)
cv2.imwrite('../data//temp.jpg', temp)
temp = cv2.imread('../data//temp.jpg', 1)
# temp2 = temp
temp = temp.tolist()
# temp2 = temp2.tolist()
finalImg = background.tolist()
...
filewa = []
filewa.append(open(DIR + filenames[0] + "-swap-points.txt","w"))
filewa.append(open(DIR + filenames[1] + "-swap-points.txt","w"))

print("detecting feature points")
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor('../extras/shape_predictor_68_face_landmarks.dat')

image1 = cv2.imread(DIR+filenames[0])
...
image2 = cv2.imread(DIR+filenames[1])
# image2 = imutils.resize(image2, width=500)
gray = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)
 
# detect faces in the grayscale image2
rects = detector(gray, 1)
...
filewa[0].close()
filewa[1].close()
print("feature points detection completed")
...
def topFlatFill(P1,P2,P3,P1lam,P2lam,P3lam, dictA, imgLeft, imgRight,t): #filling using the middle image triangle
	...
	while(y >= P3[1]):
		...
		while(x <= rightmostX):
			...
			if(newB<0):
				newB=0
			old_pix= np.uint8([[ finalImg[int(y)][int(x)] ]])
			new_pix = np.uint8([[[newR,newG,newB]]])
			old_lab_pix = cv2.cvtColor(old_pix, cv2.COLOR_BGR2LAB);
			new_lab_pix = cv2.cvtColor(new_pix, cv2.COLOR_BGR2LAB);
			# new_lab_pix[0][0][1] = old_lab_pix[0][0][1]
			# new_lab_pix[0][0][2] = old_lab_pix[0][0][2]
			new_pix_rgb = cv2.cvtColor(new_lab_pix, cv2.COLOR_LAB2BGR);

			finalImg[int(y)][int(x)] = new_pix_rgb[0][0]
			# finalImg[int(y)][int(x)] = [np.uint8(newR),np.uint8(newG),np.uint8(newB)]
			temp[int(y)][int(x)] = [255,255,255]
			# temp2[int(y)][int(x)] = [np.uint8(newR),np.uint8(newG),np.uint8(newB)]
...
def main(imgLeft,imgRight, filename1, filename2, t,t1):
	d,points1,points2 = getDict_points(filename1,filename2, t1)
	...
	for triangle in triangles:
		...
		D = getDictForP3(filename1,filename2,t1)
		if(P3[1]==P1[1]):
			#flat top triangle
			if(P3[0] < P1[0]):
...
print("working on face swap...")
main(imgLeft,imgRight,filenames[0]+"-swap-points.txt",filenames[1] + "-swap-points.txt",0.01,0.99) #sum should be 1

newImg = np.array(finalImg)

# for i in xrange(1,height-1):
# 	for j in xrange(1,width-1):
# 		temp[i][j][0] = temp[i][j-1][0]+temp[i-1][j][0]+temp[i+1][j][0]+temp[i][j+1][0]-(4*temp[i][j][0])
# 		temp[i][j][1] = temp[i][j-1][1]+temp[i-1][j][1]+temp[i+1][j][1]+temp[i][j+1][1]-(4*temp[i][j][1])
# 		temp[i][j][2] = temp[i][j-1][2]+temp[i-1][j][2]+temp[i+1][j][2]+temp[i][j+1][2]-(4*temp[i][j][2])

temp = np.array(temp)
# temp2 = np.array(temp2)
cv2.imwrite('../data//temp.jpg', temp)
# cv2.imwrite('../data//temp2.jpg', temp2)
...
cv2.imwrite('../data//output_swap.jpg', newImg)
# center = (0,0)

# output = cv2.seamlessClone(newImg, imgLeft, temp, center, cv2.NORMAL_CLONE)


# print((coordinates))
# showimg = cv2.imread('../data//output_swap.jpg',1)
